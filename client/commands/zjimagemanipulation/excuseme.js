...
module.exports.run = async (client, message, args) => {
    ...
    if (cooldown.has(message.author.id)) {
        ...
        let response = await client.util.requester(`https://obrazium.com/v1/excuseme?text=${text}`, client.config.tokens.badosz, "buffer");
        let embed = new Discord.MessageEmbed();
        embed.attachFiles([{
            attachment: response,
            name: "excuseme.png"
        }]);
        embed.setImage("attachment://excuseme.png");
        embed.setColor(ce);
